...
public abstract class ResourceBase<R extends Object, S extends Object> implements Resource<R, S> {
	...
	@Override
	public Response create(final UriInfo uriInfo, final R resource, final JsonRepresentation jsonRepresentation) {
	...
	@Override
	public Response create(final UriInfo uriInfo, final R resource) {
	...
	@Override
	public Response get(final UriInfo uriInfo, final String id, final JsonRepresentation jsonRepresentation) {

		// Get entity
		R resource = this.getService().get(id);
	...
	/**
	 * As multivalue map parameters is immutable, you must extract queryParameters,
	 * use public find operation
	 * 
	 * @param uriInfo
	 * @param fields
	 ...
	 * @return
	 */
	private Response find(final UriInfo uriInfo, final JsonRepresentation jsonRepresentation,
			MultivaluedMap<String, String> queryParameters) {

		// extract criteria
		MultivaluedMap<String, String> criteria = URIParserUtils.extractCriteria(queryParameters);

		// change criteria names with known mapping
		this.getMappingTable().getRows().forEach((oldCriterionName, newCriterionName) -> {
			List<String> criterionValues = criteria.get(oldCriterionName);
	...
	@Override
	public Response find(final UriInfo uriInfo, final JsonRepresentation jsonRepresentation) {

		Query query = new Query(uriInfo);

		return this.find(uriInfo, jsonRepresentation, query.getQueryParameters());
	...
	public Response find(final UriInfo uriInfo) {
		...
		return this.find(uriInfo, query.getRepresentation(), query.getQueryParameters());
	}

	@Override
	public Response update(final UriInfo uriInfo, final String id, final R resource,
			final JsonRepresentation jsonRepresentation) {

		R updatedResource = this.getService().update(resource);
	...
	@Override
	public Response updateWithNotNull(final UriInfo uriInfo, final String id, final R resource,
			final JsonRepresentation jsonRepresentation) {
		R resourceToUpdate = this.getService().get(id);
		...
		if ((resourceId != null) && !id.equals(resourceId)) {
			throw new FunctionalException(Functional.GENERIC, "Path id is not the same as source id");
		}
		R updatedResource = null;
		try {
			updatedResource = this.copyNonNullProperties(resource, resourceToUpdate);
			this.getService().update(updatedResource);
		} catch (Exception ex) {
			ResourceBase.LOGGER.error("Can't update resource properties", ex);
			throw new TechnicalException(Technical.GENERIC, "Can't update resource properties");
		}
	...
	public Response updateWithNotNull(final UriInfo uriInfo, final String id, final R resource) {
		...
		return this.updateWithNotNull(uriInfo, id, resource, query.getRepresentation());

	}

	@Override
	public Response merge(final UriInfo uriInfo, final String id, final JsonNode jsonPartialTarget) {

		Query query = new Query(uriInfo);

		return this.merge(uriInfo, id, jsonPartialTarget, query.getRepresentation());
	}
	...
	public Response merge(final UriInfo uriInfo, final String id, final JsonNode jsonPartialTarget,
		...
		if (id == null) {
			throw new BadUsageException(BadUsage.GENERIC, "Target id cannot be null or empty");
		}

		// Check if ids match
		final JsonNode idNode = jsonPartialTarget.get("id");
		final String partialResourceId = (idNode == null ? null : idNode.textValue());
		if (partialResourceId != null && !partialResourceId.equals(id)) {
			throw new BadUsageException(BadUsage.GENERIC, "Target id and resource representation id do not match");
		}

		// Get resource to patch
		R currentResource = this.getService().get(id);

		// Patch resource
		EditionPolicy editionPolicy = this.getEditionPolicy();
		currentResource = Jackson.merge(currentResource, jsonPartialTarget, editionPolicy);

		R patchedResource = this.getService().update(currentResource);
	...
	@Override
	public Response diff(final UriInfo uriInfo, final String id, final JsonNode jsonTarget) {

		final R currentResource = this.getService().get(id);
		final JsonNode jsonPatch = Jackson.diff(currentResource, jsonTarget, this.getEditionPolicy());
		return Response.ok(jsonPatch).build();
	}

	@Override
	public Response patch(final UriInfo uriInfo, String id, final JsonNode jsonPatch) {
	...
	public Response patch(final UriInfo uriInfo, String id, final JsonNode jsonPatch,
		...
		// Patch resource
		currentResource = Jackson.patch(currentResource, jsonPatch, this.getEditionPolicy());
	...
	protected R copyNonNullProperties(R source, R target)
	 ...
	 * @return
	 */
	protected Object getEntities(final Set<Object> resources, JsonRepresentation jsonRepresentation) {
	 ...
	 * @return
	 */
	protected Response getResponse200(final Object resource, final JsonRepresentation jsonRepresentation) {
	 ...
	 * @return
	 */
	protected Response getResponse200(final Set<Object> resources, JsonRepresentation jsonRepresentation) {
	 ...
	 * @return
	 */
	protected Response getResponse201(final UriInfo uriInfo, final R resource,
			final JsonRepresentation jsonRepresentation) {

		final UriBuilder ub = uriInfo.getAbsolutePathBuilder();
		final URI uri = ub.path(String.valueOf(this.getService().getId(resource))).build();
	...
	/**
	 * Build 201 filtered response with custom path, useful for sub resource or non
	 * relative resource
	 * 
	 * @param uriInfo
	 * @param resource
	 * @param resourcePath
	 *            a relative path from base path
	 * @param jsonRepresentation
	 * @return
	 */
	protected Response getResponse201(final UriInfo uriInfo, final Object resource, final String resourcePath,
			final JsonRepresentation jsonRepresentation) {
	 ...
	 * @return
	 */
	private Set<R> findByCriteria(MultivaluedMap<String, String> criteria) {
	 ...
	 * Convert list to set
	 */
	public Set<R> convertList(List<R> list) {
		if (list == null) {
